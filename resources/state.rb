...
action_class do
  def enforce_status
    shell_out!('getenforce').stdout.strip.downcase
  end
  ...
  def render_selinux_template(action)
    ...
    Chef::Log.info('Changes from disabled require a reboot.') if enforce_status == 'disabled' && %w(enforcing permissive).include?(action)
